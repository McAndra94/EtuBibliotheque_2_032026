 ...
 * via "cy.task".
 */
function sendCoverage(coverage, pathname = '/') {
    logMessage(`Saving code coverage for **${pathname}**`);
    const totalCoverage = (0, support_utils_1.filterFilesFromCoverage)(coverage);
    const batchSize = (0, support_utils_1.getSendCoverageBatchSize)();
 ...
 */
function sendBatchCoverage(totalCoverage, batchSize) {
    const keys = Object.keys(totalCoverage);
...
function registerHooks() {
    ...
    afterEach(() => {
        ...
        if (!hasE2ECoverage()) {
            if (hasUnitTestCoverage()) {
                ...
                const expectBackendCoverageOnly = Cypress._.get(Cypress.expose('codeCoverage'), 'expectBackendCoverageOnly', false);
    ...
    after(function collectBackendCoverage() {
        ...
        Cypress.config('baseUrl') || cy.state('window').origin;
        // @ts-ignore - proxyUrl is a runtime property
        const runningEndToEndTests = baseUrl !== Cypress.config('proxyUrl');
        const expectFrontendCoverageOnly = Cypress._.get(Cypress.expose('codeCoverage'), 'expectFrontendCoverageOnly', false);
        const specType = Cypress._.get(Cypress.spec, 'specType', 'integration');
        const isIntegrationSpec = specType === 'integration';
        if (!expectFrontendCoverageOnly &&
            runningEndToEndTests &&
            isIntegrationSpec) {
            // we can only request server-side code coverage
            // if we are running end-to-end tests,
            // otherwise where do we send the request?
            const captureUrls = Cypress._.get(Cypress.expose('codeCoverage'), 'url', '/__coverage__');
    ...
    after(function generateReport() {
        ...
        cy.task('coverageReport', null, {
            timeout: dayjs_1.default.duration(3, 'minutes').asMilliseconds(),
            log: false
...
const exposedValues = Object.fromEntries(Object.entries(Cypress.expose()).map(([key, value]) => [
    key.toLowerCase(),
    value
]));
